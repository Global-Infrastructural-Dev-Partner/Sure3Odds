...
import com.gidp.sure3odds.helper.Helper;
...
public class UsersService {
    ...
    public BaseResponse CreateNewUser(NewUser newUser) throws IOException {
        ...
        if (helper.isValidEmail(newUser.getEmail())) {
...
//        }
    ...
    public BaseResponse UpdateUser(Users users) {
        BaseResponse response = new BaseResponse();
        if (helper.isValidEmail(users.getEmail())) {
    ...
    public boolean checkEmailAddressOrPhoneNumberExist(String email, String phoneNumber) {
        ...
        Optional<Users> user = usersRepository.findByEmailOrPhoneNumber(email, phoneNumber);
        if (user.isPresent()) {
            result = true;
        }
        return result;
